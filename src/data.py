...
def load_raw_data(
    ...
    if months is None:
        #download only data for the month specified by the 'months'
        months = list(range(1, 13))
    elif isinstance(months, int):
        #download data for the entire months
        months = [months]
...
def add_missing_slots(ts_data: pd.DataFrame) -> pd.DataFrame:
    """
    Add necessary rows to the input 'ts_data' to make sure the output has a complete list of
    - pickup_hours
    - pickup_location_ids
    """
